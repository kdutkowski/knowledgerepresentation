...
\begin{itemize}
	...
	\item zapytanie kwerendą $Q$ postaci $performing\ A\ at\ t\ when\ Sc$\\ zwróci wynik $TRUE$ jeśli dla każdego modelu $S=(H,O,E)$ scenariusza $Sc$ zgodnego z $D$ zajdzie $\forall_{d \in N}\ (A,t,d) \in E$
	\item zapytanie kwerendą $Q$ postaci $performing\ A\ when\ Sc$\\ zwróci wynik $TRUE$ jeśli dla każdego modelu $S=(H,O,E)$ scenariusza $Sc$ zgodnego z $D$ zajdzie $\exists_{t \in N}\ \forall_{d \in N}\ (A,t,d) \in E$
	\item zapytanie kwerendą $Q$ postaci $performing\ at\ t\ when\ Sc$\\ zwróci wynik $TRUE$ jeśli dla każdego modelu $S=(H,O,E)$ scenariusza $Sc$ zgodnego z $D$ zajdzie $\exists_{A \in Ac}\ \forall_{d \in N}\ (A,t,d) \in E$
