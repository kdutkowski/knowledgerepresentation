...
Ten przykład pokazuje przypadek kwarendy, która pyta, czy dana akcja jest wykonywana w danym czasie.

	\subsubsection{Historia}

Mamy Billa i psa Maxa. Jeśli Bill idzie, to  Max biegnie. Jeśli Bill zatrzymuje się, Max również zatrzymuje się.
...
$(stops\_Bill,1)$  \textbf{causes} $stopping\_Max$\\
$(stops\_Bill,1)$  \textbf{invokes} $(stops\_Max,1)$  \textbf{after} $1$\\

	\subsubsection{Scenariusz}

\textit{Sc} =$(OBS,ACS)$\\
\textit{OBS} = ${\emptyset}$\\
\textit{ACS}  = ${ 	(goes\_Bill,0+1), (stops\_Bill,TODO),(goes\_Bill,TODO)	}$\\
