...
Michał jest pracującym  studentem. W środę powinien o godzinie 8.00 pojawić się w pracy zupełnie trzeźwy, a mimo to postanowił pójść do baru dzień wcześniej. Jeśli Michał się napije,  stanie się pijany. Jeśli pójdzie spać przestanie być pijany, ale stanie się skacowany, co również będzie niedopuszczalne w w jego pracy.

\subsubsection{Opis akcji}

 \textbf{initially}  \textit{drunk, hungover}\\
 \textit{(drink,2)}  \textbf{ causes}  \textit{drunk}\\
\textit{(sleep,8)}   \textbf{causes} \textit{$\neg$drunk}\\
\textit{(sleep,8)}   \textbf{causes} \textit{hungover}  \textbf{if}  \textit{drunk}\\


\subsubsection{Scenariusze}

Sc =$(OBS;ACS)$\\
OBS =  ${(drunk=FALSE,hungover=FALSE,10)}$\\
...
Scenariusz \textit{ Sc2} jest w pełni poprawny i wykonywalny. Scenariusza \textit{Sc} nie można wykonać, ponieważ wymaga on by w chwili 10 fluenty textit{drunk} i \textit{hungover} miały wartość FALSE, jednak w tej chwili zmienna \textit{hungover} ma wartość TRUE.
...
$(switches\_on\_computer,1)$ \textbf{causes} $\neg switch\_on\_computer$\\
$(disconnects\_power,1)$ \textbf{causes} $\neg connect\_power\_computer$ \textbf{and} $\neg swith\_on\_computer$ \\
