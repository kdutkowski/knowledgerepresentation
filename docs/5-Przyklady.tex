...
Sc =$(OBS,ACS)$\\
OBS =  $ {\{(drunk=FALSE,10), (hungover=FALSE,10)\} }$\\
...
Scenariusz \textit{ Sc2} jest w pełni poprawny i wykonywalny. Scenariusza \textit{Sc} nie można wykonać, ponieważ wymaga on by w chwili 10 fluenty \textit{drunk} i \textit{hungover} miały wartość FALSE, jednak w tej chwili zmienna \textit{hungover} ma wartość TRUE.
...
\subsubsection{Scenariusz}
 ...
 \textit{Sc} =$(OBS,ACS)$\\
 \textit{OBS} =  ${\emptyset}$\\
 \textit{ACS} = ${\{((making\_panc,2),0), ((making\_cake,2)2)\}}$
