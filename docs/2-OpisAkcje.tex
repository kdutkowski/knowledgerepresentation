...
\begin{definition}Semantyczną strukturą języka $AL$ nazywamy system $ S=(H,O,E) $ zgodny z dziedziną $D$ taki, że:
\begin{itemize}
...
\end{definition}
 ...
 Niech: $A,B$ będą akcjami, $ f $ - fluentem, $\alpha, \pi$ - będą formułami, $d,d_{2},d_{3} $ - liczbami naturalnymi (oznaczającymi czas trwania akcji) oraz $fl(\alpha)$ będzie zbiorem fluentów występujących w $\alpha$. Wtedy dla zdań języka $AL$ muszą być spełnione następujące warunki: 
   ...
   \end{itemize}
   \begin{definition}
   Niech $S = (H,O,E)$ będzie strukturą języka $AL$, $ Sc=(OBS,ACS) $ będzie scenariuszem, oraz $ D $ dziedziną. Powiemy, że $ S $ jest strukturą dla $Sc$ zgodną z opisem domeny $D$, jeśli:
     \begin{itemize} 
